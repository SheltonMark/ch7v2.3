 ...
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_GetStreamBufInfo(VENC_CHN VeChn, VENC_STREAM_BUF_S *pstStreamBufInfo);
 ...
 * @attention:1. JPEG编码通道支持,H26x编码通道仅HwDirect模式下支持。
 ...
 * @attention: JPEG编码通道支持,H26x编码通道仅HwDirect模式下支持。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @return {*}: 0：成功，非0：失败
 */
...
/**
* @description: 送帧编码并且当前编码帧使用对应的QpMap表。
* @attention:1. 送YUV图像太快可能导致编码来不及从而导致接口调用超时。
...
/**
 * @description: 设置H26X去呼吸效应配置参数。
 * @attention:1. 。
           2. 。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_DEBREATHEFFECT_S} *pstDeBreathEffect: 输入，编码通道去呼吸效应参数结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_SetDeBreathEffect(VENC_CHN VeChn, const VENC_DEBREATH_EFFECT_S *pstDeBreathEffect);

/**
 * @description: 获取H26X去呼吸效应配置参数。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_DEBREATHEFFECT_S} *pstDeBreathEffect: 输出，编码通道去呼吸效应参数结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_GetDeBreathEffect(VENC_CHN VeChn, VENC_DEBREATH_EFFECT_S *pstDeBreathEffect);

/**
 * @description: 设置H26X去模糊效应配置参数。
 * @attention:1. 。
           2. 。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_DEBLURRING_S} *pstDeBlurringEffect: 输入，编码通道去模糊效应参数结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_SetDeBlurringEffect(VENC_CHN VeChn, const VENC_DEBLURRING_EFFECT_S *pstDeBlurringEffect);

/**
 * @description: 获取H26X去模糊效应配置参数。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_DEBLURRING_S} *pstDeBreathEffect: 输出，编码通道去模糊效应参数结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_GetDeBlurringEffect(VENC_CHN VeChn, VENC_DEBLURRING_EFFECT_S *pstDeBlurringEffect);

/**
 * @description: 设置H26X通道的Slice分割属性。
 * @attention:1. 。
           2. 。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_SLICE_SPLIT_S} *pstDeBlurringEffect: 输入，H.26x码流slice分割参数
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_SetSliceSplit(VENC_CHN VeChn, const VENC_SLICE_SPLIT_S *pstSliceSplit);

/**
 * @description: 获取H26X通道的Slice分割属性。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_SLICE_SPLIT_S} *pstDeBlurringEffect: 输出，H.26x码流slice分割参数
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_GetSliceSplit(VENC_CHN VeChn, VENC_SLICE_SPLIT_S *pstSliceSplit);

/**
 * @description: 用设置编码通道的区域MV属性。
 * @attention:1.。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_MV_CFG_S} *pstMvfg: 输入，编码通道区域MV属性结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_SetMvRegion(VENC_CHN VeChn, const VENC_MV_CFG_S *pstMvfg);

/**
 * @description: 获取编码通道的区域MV属性。
 * @param {VENC_CHN} VeChn: 输入，编码通道号， 取值范围：[0, VENC_MAX_CHN_NUM)
 * @param {VENC_MV_CFG_S} *pstMvfg: 输出，编码通道区域MV属性结构体指针
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_VENC_GetMvRegion(VENC_CHN VeChn, VENC_MV_CFG_S *pstMvfg);
