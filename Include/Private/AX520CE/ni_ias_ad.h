...
typedef struct tagIasAdModel {
    ...
    union {
        IAS_AD_CFG_S stAdModelCfg;                 /* UNION:enType=IAS_AD_MODEL_READ_FROM_MEM | AD 算法模型配置 */
        NI_CHAR      acName[IAS_ALG_MAX_NAME_LEN]; /* UNION:enType=IAS_AD_MODEL_READ_FROM_FILE | AD 算法模型路径 */
    };
} IAS_AD_MODEL_S;

typedef struct tagIasAdCallBackFunc {
    NI_S32(*pfn_ias_ad_result)
    (IAS_ALG_CHN AdChn, IAS_AD_RESULT_S *pstResult); /* 用于获取检测结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
