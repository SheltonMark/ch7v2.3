...
typedef struct tagIasPtdModel {
    ...
    union {
        IAS_PTD_CFG_S stPtdModelCfg;                /* UNION:enType=IAS_PTD_MODEL_READ_FROM_MEM | PTD 算法模型配置 */
        NI_CHAR       acName[IAS_ALG_MAX_NAME_LEN]; /* UNION:enType=IAS_PTD_MODEL_READ_FROM_FILE | PTD 算法模型路径 */
    };
} IAS_PTD_MODEL_S;

typedef struct tagIasPtcInfo {
    NI_S32 s32PtcScore; /* 宠物置信度 */
    NI_U32 au32Rev[3];
} IAS_PTC_INFO_S;
...
typedef struct tagIasPtcObjResult {
    IAS_ID_INFO_S  stIdInfo;  /* 目标ID信息 */
    IAS_RECT_S     stRect;    /* 目标在原图上的位置 */
    IAS_PTC_INFO_S stPtcInfo; /* 宠物信息 */
    IAS_OBJ_TYPE_E enType;    /* 目标类型*/
    NI_U32         au32Rev0[3];
    NI_U32         u32PtcStauts; /* 宠物 状态信息 1为新增ID，2为现有ID */
    NI_U16         u16Status;    /* 跟踪的状态 0：临时目标   1：目标创建  2：目标更新  3：目标丢失（目标隐藏）  4：目标删除， 跟踪关闭时无效*/
    NI_U16         au16Rev[5];
    NI_U64         u64Timestamp; /* 当前目标对应时间戳 */
    NI_U32         au32Rev1[2];
...
typedef struct tagIasPtdCallBackFunc {
    NI_S32(*pfn_ias_ptd_result)
    (IAS_ALG_CHN PtdChn, IAS_PTD_RESULT_S *pstResult); /* 用于获取检测结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32(*pfn_ias_ptc_result)
    (IAS_ALG_CHN PtdChn, IAS_PTC_RESULT_S *pstResult); /* 用于获取抓拍结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32(*pfn_ias_release_frame)
    (IAS_ALG_CHN PtdChn, IAS_TASK_IMAGE_S *pstFrame); /* 释放frame， 如果用户注册了该函数， 有结果时会调用该函数, 如果不注册则需要调用接口获取 */
