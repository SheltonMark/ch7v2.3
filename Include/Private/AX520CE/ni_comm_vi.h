...
typedef enum tagViDownMode {
    VI_DOWN_DEL_CURRENT   = 0, /* 删除当前帧，eg:step为2时,删除当前帧保留后续2帧，帧率变为原来的2/3 */
    VI_DOWN_PICK_ONE      = 1, /* 提取当前帧（仅一帧，仅测试模式）*/
    VI_DOWN_PICK_CONTINUE = 2, /* 提取当前帧（连续），eg:step为2时,保留当前帧删除后续2帧，帧率变为原来的1/3*/
} VI_DOWN_MODE_E;
...
typedef struct tagViSenExchangeCfg {
    NI_BOOL bExChgModeEn;    /*sen切换使能*/
    NI_U16  u16IntervalFrms; /*发送多少帧后切换*/
} VI_SEN_EXCHANGE_S;

/* the attributes of a Video interface device */
typedef struct tagViDevAttr {
    VI_INTF_TYPE_E enIntfType;      /* Interface type */
    NI_U32         u32DataSelMask;  /* range:[0, 0xffff], 低16bit有效，VI 16根线与sensor数据线的连接关系，若第X根线与sensor的有效数据线相连，
                                               则将第Xbit置1, 否则置0, X取值0~15 */
    NI_BOOL bDataRev;               /* 特殊情况，sensor的硬件走线与vi
                                       数据线接反时，如sensor_data0与VI_data7相连时，此处配true，反之false*/
    BAYER_SEQ_E       enRgbSeq;     /* 表示sensor输入的RGB格式排列顺序 */
    ...
    VI_SEN_EXCHANGE_S stSenExCfg;   /*sensor切换配置*/
    NI_BOOL           bRingModeEn;  /* ring buf模式使能 */
} VI_DEV_ATTR_S;
...
typedef struct tagViWorkParam {
    ...
    SIZE_S             stColorDestSize; /*彩条输出总宽高*/
} VI_WORK_PARAM_S;
...
typedef struct tagMipiDataTiming {
    ...
    NI_U32
    au32DataTypeCode[3]; /* clipper 暂不适用,HDR模式下不同曝光帧的数据类型，MIPI_HDR_DATA_IDENTIFY_DATATYPE时有效 */
...
typedef struct tagViRawDataInfo {
    ...
    NI_U32
    u32PhyAddr; /* 软件导数据源的基地址, 16byte对齐, 每一帧数据存储空间的大小, 16byte对齐，宽动态按照短、中、长顺序排列。
                 */
} VI_RAW_DATA_INFO_S;
