 ...
 * @param {NI_CHAR} *pszMmzName：输入，要从哪个 MMZ 分区中申请内存。如MMZ分区中有"anonymous"分区，则本项参数可配为 NULL。
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_SYS_MmzAlloc(NI_U32 *pu32PhyAddr, NI_VOID **ppVirAddr, NI_U32 u32Size,
    const NI_CHAR *pszMmbName, const NI_CHAR *pszMmzName);
 ...
 * @return {*}: 0：成功，非0：失败
 */
NI_S32 NI_MDK_SYS_MmzAllocCached(NI_U32 *pu32PhyAddr, NI_VOID **ppVirAddr, NI_U32 u32Size,
    const NI_CHAR *pszMmbName, const NI_CHAR *pszMmzName);
