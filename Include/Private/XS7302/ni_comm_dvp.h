...
{
    NI_U32      u32KrY;      /* YUV转R分量的系数0 默认：0x199 */
    NI_U32      u32KrV;      /* YUV转R分量的系数1 默认：0x12a */
    NI_U32      u32KgY;      /* YUV转G分量的系数0 默认：0x79c */
    NI_U32      u32KgU;      /* YUV转G分量的系数1 默认：0x12a */
    NI_U32      u32KgV;      /* YUV转G分量的系数2 默认：0x12a */
    NI_U32      u32KbY;      /* YUV转B分量的系数0 默认：0x730 */
...
typedef struct tagDvpCmd
{
    ...
    NI_U32                    u32FilterHalfWin;      /* 缩放时滤波窗口，range:[0, 3]，默认为3。XS7320不支持 */
...
typedef struct tagDvpCmdArray
{
    ...
    //NI_S32              s32JobHandle;           /* 以JOB任务方式提交命令时使用，传入合法的job handle，非JOB任务方式时，该值填NI_ERR_DVP_INVALID_HANDLE,主要用于
    //                                               NI_MDK_DVP_WaitJobDone等待硬件完成具体任务操作 */
}DVP_CMD_ARRAY_S;
