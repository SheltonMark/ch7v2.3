...
/**
 * @description: 用户送图像数据给VI及后级模块。用于灌源发送指定数量的图片，会循环发送。非阻塞接口，在设备使能前调用。只有设备0支持该功能
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
...
/**
 * @description: 用户送图像数据给VI及后级模块，单帧发送。用于灌源发送指定数量的图片，仅发送一次。阻塞接口，在设备使能后调用,仅能在非直通模式下工作。只有设备0支持该功能
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
...
/**
 * @description: 启动VI设备捕获图像功能。开启该功能，VI驱动会实时抓取Bayer、YUV图像。该接口调用前，需提前设置设备属性并禁用设备。直通模式下，该功能不支持
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
...
/**
 * @description: 配置设备与VI内部编解码通道映射关系。该接口仅在MIPI传输YUV数据的时候需要用户调用配置编解码通道需要在NI_MDK_VI_SetDevAttr之前调用
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
...
/**
 * @description: 设置中断回调函数。通过该接口注册的回调会在 VI 中断中被调用，因此不能在注册函数中睡眠。函数执行时间要求尽可能的短
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
...
/**
 * @description: 设置距离sensor帧头多少行触发中断。接收长曝光 u32TriggerLine 行后，调用用户通过 NI_MDK_VI_SetIntCallBack 接口注册的回调函数
 * @param {VI_DEV} ViDev: 输入，vi设备号。取值范围：[0,VI_MAX_DEV_NUM)
