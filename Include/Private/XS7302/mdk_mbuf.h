 ...
 * @Copyright: : (C) 2022 ZheJiang Xinsheng Electronic Technology Co., Ltd. All rights reserved.
 */
...
/**
 * @description: 初始化媒体缓存池模块，并按照NI_MDK_MBUF_SetConf接口中的配置创建公共缓存池。调用本接口前需先调用NI_MDK_MBUF_SetConf接口。
...
/**
 * @description: 去初始化缓存池模块，将所有私有和公共缓存池资源销毁(强制销毁)。需确保没有任何人在使用缓存池功能时，才能调用本接口，否则会出现未定义的错误。
 *               重复调用本接口将返回成功。
 * @return {*}：0：成功，非0：失败
 */
...
/**
 * @description: 创建一个缓存池，缓存池的大小为u32BlkCnt*u32BlkSize，会自动向上对齐到4KB，不用时需要调用NI_MDK_MBUF_DestroyPool接口销毁。
 * @param {NI_U32} u32BlkSize: 输入，缓存块大小，单位:byte，缓存池的总大小应控制在mmz驱动管理的内存范围内，超出MMZ的可用空闲内存将返回失败。
...
/**
 * @description: 从公共或私有缓存池中获取一块指定大小的空闲缓存块,调用本接口获取到的缓存块，在不用时，需要调用NI_MDK_MBUF_ReleaseBlock接口释放。
 * @param {MBUF_POOL} PoolId: 输入，缓存池ID号，若为MBUF_INVALID_POOLID则表从公共缓存池中获取
 * @param {NI_U32} u32BlkSize: 输入，期望获取到的缓存大小。当PoolId为指定缓存池时，该参数无效，当PoolId为MBUF_INVALID_POOLID时，需要配置该参数，从公共缓存池组中获取大于等于u32BlkSize的空闲缓存块。
 * @param {NI_CHAR} *pcMmzName：输入，当从公共缓存池中获取时，指定从哪片ddr的公共池子中获取;当从指定缓存池中获取时，该参数无效。
...
/**
 * @description: 对缓存块占用计数做加1操作。一个缓存块可以被多次占用，不用时，要配对地调用NI_MDK_MBUF_UserSub接口对占用计数做减1操作。
 *               对一个空闲的缓存块做加操作将返回失败。
...
/**
 * @description: 对缓存块的占用计数做减1操作。当占用计数减为0时，将变成空闲块，以待下一次GetBlock时被分配使用。等效于NI_MDK_MBUF_ReleaseBlock接口。
...
/**
 * @description: 映射缓存池虚拟地址。创建的缓存池默认是未映射虚拟地址的，如果CPU要访问，可调用本接口映射虚拟地址，之后调用NI_MDK_MBUF_GetBlkVirAddr
...
/**
 * @description: 释放已映射的缓存池虚拟地址。用户需确保没有任何人再使用缓存池虚拟地址的情况下，才能调用本接口释放虚拟地址，否则将可能发生未定义的错误。
 *               重复调用本接口将返回成功。
 * @param {MBUF_POOL} PoolId: 输入，缓存池ID
 * @return {*}: 0：成功，非0:失败
 */
 ...
 * @param {MBUF_POOL} PoolId: 输入，缓存池ID，该缓存池需是已经映射过虚拟地址的，否则会返回失败，可用调用NI_MDK_MBUF_MmapPool接口映射缓存池用户态虚拟地址。
