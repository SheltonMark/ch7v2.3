...
typedef struct tagIasFcModel
{
    ...
    union 
    {
        IAS_FC_CFG_S       stFcModelCfg;                        /* FC 算法模型配置 */
        NI_CHAR            acName[IAS_ALG_MAX_NAME_LEN];        /* FC 算法模型路径 */
    };
} IAS_FC_MODEL_S;
...
{
    NI_S32 (*pfn_ias_fd_result)(IAS_ALG_CHN FcChn, IAS_FD_RESULT_S *pstResult);       /* 用于获取检测结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32 (*pfn_ias_fc_result)(IAS_ALG_CHN FcChn, IAS_FC_RESULT_S *pstResult);       /* 用于获取抓拍结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32 (*pfn_ias_fp_result)(IAS_ALG_CHN FcChn, IAS_FC_PG_RESULT_S *pstResult); /* 用于获取抓拍结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32 (*pfn_ias_release_frame)(IAS_ALG_CHN FcChn, IAS_TASK_IMAGE_S *pstFrame);   /* 释放frame， 如果用户注册了该函数， 有结果时会调用该函数, 如果不注册则需要调用接口获取 */
