...
typedef struct tagIasOdModel
{
    ...
    union 
    {
        IAS_OD_CFG_S       stOdModelCfg;                        /* OD 算法模型配置 */
...
{
    NI_S32 (*pfn_ias_od_result)(IAS_ALG_CHN OdChn, IAS_OD_RESULT_S *pstResult);      /* 用于获取检测结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32 (*pfn_ias_pg_result)(IAS_ALG_CHN OdChn, IAS_PG_RESULT_S *pstResult);      /* 用于获取周界结果， 如果用户注册了该函数， 算法有结果时会调用该函数, 如果不注册则需要调用接口获取 */
    NI_S32 (*pfn_ias_release_frame)(IAS_ALG_CHN OdChn, IAS_TASK_IMAGE_S *pstFrame);     /* 释放frame， 如果用户注册了该函数， 有结果时会调用该函数, 如果不注册则需要调用接口获取 */
