...
extern int isp_init(SAMPLE_VI_INFO_S viInfo);

/**
 * @brief Convert driver-level SEN_TYPE to application-level ISP_SENSORTYPE_E
 * @param sen_type Driver-level sensor type (SEN_TYPE_GC4653, etc.)
 * @return Application-level sensor type, or -1 if not found
 *
 * This helper function looks up the sensor configuration table to find
 * the matching ISP_SENSORTYPE_E for a given SEN_TYPE value.
 */
static ISP_SENSORTYPE_E ConvertSenTypeToSensorType(int sen_type)
{
    int count = 0;
    const SensorConfig_t* all_sensors = SensorConfig_GetAll(&count);

    for (int i = 0; i < count; i++) {
        if (all_sensors[i].sen_type == sen_type) {
            return all_sensors[i].sensor_type;
        }
    }

    return (ISP_SENSORTYPE_E)-1;  /* Not found */
}
...
int VideoPipeline_Init(void)
{
    ...
    /* Convert driver-level SenType to application-level sensor_type */
    PRINT_INFO("SenType conversion: BEFORE=%d (SEN_TYPE_*)\n", pSensorDevice->SenType);

    ISP_SENSORTYPE_E app_sensor_type = ConvertSenTypeToSensorType(pSensorDevice->SenType);
    if (app_sensor_type == (ISP_SENSORTYPE_E)-1) {
        PRINT_ERROR("Cannot convert SenType %d to ISP_SENSORTYPE_E\n", pSensorDevice->SenType);
        return -1;
    }

    PRINT_INFO("SenType conversion: AFTER=%d (ISP_SENSORTYPE_E)\n", app_sensor_type);

    /* Get sensor-specific parameters from sensor_config module */
    const SensorConfig_t* sensor_cfg = SensorConfig_Get(app_sensor_type);
    if (!sensor_cfg) {
        PRINT_ERROR("Unsupported sensor type: %d\n", app_sensor_type);
        return -1;
    }

    stViInfo.stSenCfg.u32DevAddr = sensor_cfg->i2c_dev_addr;
    stViInfo.stSenCfg.u32ClkFreq = sensor_cfg->clk_freq_khz;

    PRINT_INFO("VI Config: %dx%d %dfps sensor=%s (type=%d) i2c=0x%x clk=%dkHz\n",
               stViInfo.stSenCfg.u32Width,
               stViInfo.stSenCfg.u32Height,
               stViInfo.stSenCfg.u32Fps,
               sensor_cfg->name,
               stViInfo.stSenCfg.enSenType,
               sensor_cfg->i2c_dev_addr,
               sensor_cfg->clk_freq_khz);
